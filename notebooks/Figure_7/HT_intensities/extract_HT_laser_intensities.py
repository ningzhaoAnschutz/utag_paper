"""
Extract Laser Intensities from Harringtonine LIF Files

This script processes each subfolder in the Harringtonine data directory
independently, extracting laser intensity metadata from all LIF files and
saving a separate Excel file per folder.

Usage:
    python extract_HT_laser_intensities.py
...
# ── Configuration ──────────────────────────────────────────────────────────────
DATA_ROOT = Path("/Volumes/Luis_DRIVE/UTag_paper_data/Harringtonine")
...
if __name__ == "__main__":
    print("=" * 70)
    print("Laser Intensity Extraction — Harringtonine Dataset")
    print("=" * 70)
    print(f"\nData root  : {DATA_ROOT}")
